...
?>
<?php
          $sql = "SELECT * FROM news where id='$id_event'";
          $result = mysqli_query($con,$sql); 
          while($row = mysqli_fetch_assoc($result)){
            $img = $row['news_img'];
            $recordname = $row['news_name'];
            $date = $row['news_date'];
            }
?> 
<!DOCTYPE html>
    ...
    <!-- Book show -->
      <div class="row pt-5 mt-3 mb-5">
        <div class="col-sm-12 mb-1">
          <h3 class="display-4 mt-auto" style="width:15%;font-size: 1.6rem;float: left;"><i class="fas fa-microphone-alt text-light fa-1x mr-3 text-dark mr-3"></i><strong class="pt-5">News</strong></h3> <a href="index.php" class="btn btn-success"><i class="fas fa-plus-circle pr-2 icon"></i>Add News</a>
        <hr>
        </div>
      <!-- Main content -->
      <div class="col-sm-12 mb-1">
      ...
      <div class="col-sm-6 mb-2 bg-white background pt-3 mr-3">
		    ...
		    <input type="text" name="name" value="<?php echo $recordname;?>" placeholder="" class="form-control mb-2">
		   ...
		   <img src="<?php echo "../../".$img;?>" class="img-fluid w-100 mb-2" style="border:1px solid lightgray;" alt="image" width="100" height="80">
		   <div class="mt-2 background w-100" >
        <input type="file" name="image" class="form-control-file mb-0  p-2">
		   </div>
		   <input type="submit" name="editnews" value="Edit" class="btn btn-primary w-50 mt-5 small-btn mb-2">
		   </div>
      </div> 
      </form>   
      </div>
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        ...
        if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" ) {
         ...
         $uploadOk = 0;
           ...
           {
            $sql_event = "update news set news_img ='$send_img',news_name='$send_name',news_date='$send_date' where id='$id_event'";
